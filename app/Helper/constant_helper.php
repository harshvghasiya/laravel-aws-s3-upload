...
// File prefix
function FILE_PREFIX_KEYWORD()
{
    return 'file';
}

// File route name
function FILE_ROUTE_NAME()
{
    return 'file.';
}
...
{   
    $folder = \App\Models\Folder::where('id', $id)->first();

    return true;

    $files = \App\Models\FileMan::where('folder_id', $id)->get();

    if ( $folder != null && !$files->isEmpty() ) {
        
        foreach ($files as $ke => $val) {
            
            \Storage::disk('s3')->delete($val->name);
            $val->delete();
        }
    }
    $sub_folder = \App\Models\SubFolder::where('parent_id', $folder_id)->get();
