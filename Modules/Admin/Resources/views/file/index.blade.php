        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="col-md-6">
                    ...
                    <button class="btn btn-info text-white upload_file" >Upload File</button>
                </div>
            </div>
        </div>
        <div class="card">
      ...
      @if( !$folders->isEmpty() )
        @foreach( $folders as $ke => $folder)
          <div class="tile folder main_folder">
            ...
            <div>  <button type="button" class="btn edit_folder btn-sm btn-warning">Edit</button> <button class="btn btn-sm delete_folder btn-danger" data-id="{{ $folder->id }}" type="button">Delete</button>  </div>
          </div>
        @endforeach
        @if( isset( $files ) && !$files->isEmpty())
            ...
            @foreach( $files as $k => $file )
                ...
                <div class="tile folder main_folder">
                  ...
                  <div>   <button class="btn btn-sm btn-danger delete_file" data-id="{{ $file->id }}" type="button">Delete</button>  </div>
                </div>
            @endforeach
        @endif
      @endif
        ...
        <div class="modal fade" id="add_folder" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <form class="FromSubmit" action="{{ route('admin.file.store_folder') }}" id="add_folder" />
        ...
        <div class="modal fade" id="upload_file" tabindex="-1" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="add_sfile">Add File</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <form class="FromSubmit" action="{{ route('admin.file.store_file') }}" id="add_file" />
              <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                          ...
                          <span class="text-danger error_form" id="file_name_error"></span>
  ...
  $(document).ready(function() {
      ...
      $(document).on('click', '.delete_file', function(event) {
          ...
          //   if (result.isConfirmed) {
              ...
              $.ajax({
                  url: '{{ route('admin.file.delete_file') }}',
                  type: 'POST',
      ...
      $(document).on('click', '.delete_folder', function(event) {
          ...
          //   if (result.isConfirmed) {
              ...
              $.ajax({
                  url: '{{ route('admin.file.delete_folder') }}',
                  type: 'POST',
